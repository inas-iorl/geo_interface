...
const headers = ref([
  ...
  {title: 'measure', align: 'start', sortable: false, key: 'measure'},
  {title: 'parent_path', align: 'start', sortable: false, key: 'parent_path'},
  {title: 'parent_id', align: 'start', sortable: false, key: 'parent_id'},
  {title: 'category_id', align: 'start', sortable: false, key: 'category_id'},
...
async function load() {
  loading.value = true
  await elementsStore.getDataTypes()
  totalItems.value = elementsStore.data_types.length
  loading.value = false
}

function editItem (item) {
  editedIndex.value = elementsStore.data_types.indexOf(item)
...
function deleteItemConfirm () {
  elementsStore.data_types.splice(this.editedIndex, 1)
  closeDelete()
}
      ...
      <v-toolbar flat>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" md="4" sm="6">
                    <v-text-field v-model="editedItem.measure" label="Calories"></v-text-field>
                  </v-col>
                  <v-col cols="12" md="4" sm="6">
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue-darken-1" variant="text" @click="close">Отменить</v-btn>
