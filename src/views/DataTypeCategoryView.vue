...
async function load() {
  loading.value = true
  await elementsStore.getDataTypeCategory()
  totalItems.value = elementsStore.data_type_category.length
  loading.value = false
}

function editItem (item) {
  editedIndex.value = elementsStore.data_type_category.indexOf(item)
...
function deleteItemConfirm () {
  elementsStore.data_type_category.splice(this.editedIndex, 1)
  closeDelete()
}
...
<!--   ></v-data-table-server>-->
  ...
  <v-data-table
    :headers="headers"
    :items="elementsStore.data_type_category"
  >
    <template v-slot:top>
      <v-toolbar flat>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ props }">
            <v-btn class="mb-2" color="primary" dark v-bind="props">Новая категория</v-btn>
          </template>
          <v-card>
            <v-card-title>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" md="4" sm="6">
<!--                    <v-text-field v-model="editedItem.names" label="Названия"></v-text-field>-->
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="text-h5">уверенны??</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
